...
jest.mock('../main/clients/ExpoPushClient', () => {
    return {
        ...
        //@ts-ignore
        pushToClients: jest.fn().mockImplementation(()=>expoClient.pushToClients(['any'], 'test'))
    }
})
...
describe("ChatService - chat operations", () => {
    ...
    test('3 Test invalid flat has not matched', async () => {
        try {
            ...
            expect(e.message).toEqual('Associated flat has not yet matched with this profile - cannot create chat')
        }
    });
    ...
    test("4 Test valid request", async () => {
        ...
        expect(spyAddMemberships).toHaveBeenCalledWith(chatId, profileId, ['123-advertising', '456'])
    ...
    test('1 Test invalid user is not member of chat request', async ()=>{
        try {
            ...
            expect(e.message).toEqual('User is not a member of this chat - cannot delete the chat')
        }
    })
    ...
    test('1 Test onMessageCreate function', async()=>{
        ...
        expect(spyPushToClients).toHaveBeenCalledWith(['expo', 'expo'], {title: 'Roomeight', body: 'testName: testText'});
        const updates: any = {};
        updates['/lastSender'] = 'testName'
        updates['/lastMessage'] = 'testText'
        updates['/timestamp'] = 1
        expect(spyUpdateChatInfo).toHaveBeenCalledWith(context.params.chatId, updates)
    })
    ...
    test('1 Test onChatPresenceUpdate initator is user online', async()=>{
        ...
        updates['/presence/user/lastChanged'] = admin.database.ServerValue.TIMESTAMP;
    ...
    test('2 Test onChatPresenceUpdate initator is flat alone online', async()=>{
        ...
        updates['/presence/flat/lastChanged'] = admin.database.ServerValue.TIMESTAMP;
